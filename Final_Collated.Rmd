...
- `Bailing`, `Bailing Lvl` and `Density` are bimodal. To a less extent, so is `Air Pressurer`.
...
### Correlation
We are using a correlation matrix to help us better understand the relationships between predictor variables. We are leaving the target (`PH`) in the dataset so that we may learn of predictors with which it has correlation. Perhaps for no other reason than curiosity, and so that we don't remove predictors with which it has strong relationships.
